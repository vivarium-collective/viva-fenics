...
def main() -> int:
    ...
    try:
        register_run(
            STUDY_DIR / "runs.db",
            run_id=f"navier-stokes-baseline-{int(started)}",
            ...
            params={
                ...
                "quasi_steady_rel_change": quasi_steady_rel_change,
                "mean_abs_divergence": mean_abs_divergence,
                "max_speed_across_sweep": max_speed_across_sweep,
            },
            n_steps=len(speeds),
            started_at=started,
        ...
        print(f"[navier-stokes] run recorded in {STUDY_DIR / 'runs.db'}")
    except Exception as exc:  # noqa: BLE001 -- runs.db recording is best-effort
        print(f"[navier-stokes] warning: runs.db registration skipped: {exc}", file=sys.stderr)
